...
print("=== Puertos MIDI disponibles (IN) ===")("Python ejecutándose desde:", sys.executable)
...
for msg in inport:
    ...
    # -------------------------------------------------
    if msg.type == "note_on" and msg.velocity > 0 and msg.note in GRID_NOTES:
    ...
    if msg.type in ("note_on", "note_off") and getattr(msg, "note", None) in GRID_NOTES:
        ...
        # CC = nota + layer * NUM_PADS  (0–39, 40–79, 80–119, ...)
        base_cc = msg.note + current_layer * NUM_PADS
        ...
        print(
            ...
            f"→ CC{base_cc} ch0 (value {cc_value})"
        )
